...
#         save_info[idx] = p_info.split()
#     for key in save_info.keys():
#         save_info[key].sort()
#     for qur in query:
#         qur = qur.replace('and','')
#         qur = qur.replace('  ',' ')
#         tmp = [i for i in range(len(save_info))]
#         req = qur.split(' ')
#         for i,each_info in enumerate(req):
#             save_things_to_delete = []
#             if each_info == '-':
#                     continue
#             for j in tmp:
#                 if i==4:
#                     if int(each_info)>int(save_info[j][i]):
#                         save_things_to_delete.append(j)
#                     continue
#                 elif each_info != save_info[j][i]:
#                     save_things_to_delete.append(j)
#             for trash in save_things_to_delete:
#                 tmp.remove(trash)
